...
import type { DaGamaBoardSummary } from '@/plugins/canvas/dagama/api';
import type { DaGamaBoard } from '@/plugins/canvas/dagama/board';
import { DAGAMA_RUN_STATUS_LABEL } from '@/plugins/canvas/dagama/runs';
import type {
  DaGamaRun,
  DaGamaRunPreview,
...
function StatusDot({ status }: { status: DaGamaRunStatus }) {
 ...
 * The run selector.
 *
 * A chip rather than a tab row because a board can have many runs and only one
 * of them is being watched — the point is to say which.
 */
            ...
            >
              <div className="flex w-full items-center gap-2">
                ...
                <span className="text-muted-foreground text-[10px]">
                  {DAGAMA_RUN_STATUS_LABEL[run.status]}
                </span>
              </div>
...
export function BoardMenu({
  ...
  onOpen: (summary: DaGamaBoardSummary) => void;
  onDelete: (summary: DaGamaBoardSummary) => void;
  onNew: () => void;
  onSaveAs: () => void;
}) {
  return (
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <span className="font-mono">.coslash/dagama/boards</span>. Runs execute in their own clone, so
            this folder&apos;s working tree is never touched.
          </DialogDescription>
        </DialogHeader>
        <Input
          value={path}
...
export function RunDialog({
  ...
  starting: boolean;
  onOpenChange: (open: boolean) => void;
  onPreview: (input: { boardId: string } | { board: DaGamaBoard }) => Promise<DaGamaRunPreview>;
  onStart: (input: { source: DaGamaRunSourceInput; boardName?: string }) => Promise<void>;
}) {
  ...
  const [preview, setPreview] = useState<DaGamaRunPreview | null>(null);
  const [previewError, setPreviewError] = useState('');
  const [error, setError] = useState('');
  const [pending, setPending] = useState(false);
  const busy = starting || pending;

  // Preflight only reads publish.base from the board, so the effect is pinned to
  // that: unrelated card edits must not spam preview while the dialog is open.
  const draftBase = board.components.publish.publish?.base ?? '';
  ...
  useEffect(() => {
    ...
    // `board` is intentionally omitted: only publish.base affects the preflight,
    // and depending on the object would re-run on every keystroke elsewhere.
 ...
 * would turn a ticket body into same-origin markup.
 */
