...
describe('retryRemoteRefreshAndWait', () => {
  ...
  it('waits for an in-flight refresh when retry is throttled', async () => {
    vi.stubGlobal('window', {
      location: { hash: '', pathname: '/', search: '' },
      history: { state: null, replaceState: vi.fn() },
      sessionStorage: { getItem: vi.fn(() => null), setItem: vi.fn() },
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        Response.json({ error: 'busy', code: 'remote_retry_throttled' }, { status: 429 }),
      )
      .mockResolvedValueOnce(Response.json(readyMachine));
    vi.stubGlobal('fetch', fetchMock);

    await expect(retryRemoteRefreshAndWait()).resolves.toEqual(readyMachine);
    expect(fetchMock.mock.calls[1]?.[0]).toBe('/api/remote/status');
  });
});
