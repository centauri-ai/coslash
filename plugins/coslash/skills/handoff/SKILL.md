...
On success, make the entire final response the exact command stdout, starting with its `# Handoff` heading. Do not add a preface, summarize, or reformat it. If the command fails, return stderr and the exit status, then suggest `coslash doctor --json`.
