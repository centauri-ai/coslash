...
Run `coslash review <agent>:<session> --with claude|codex|opencode` with the `selector` returned by `coslash sessions` and the user's reviewer. If the user did not specify a reviewer, ask which one to use; do not choose or fall back automatically.
