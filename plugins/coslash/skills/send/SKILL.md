...
Run `coslash send <agent>:<session> --to claude|codex [message]` with the `selector` returned by `coslash sessions`, the user's target, and optional message. Pass the message as one quoted argument.
