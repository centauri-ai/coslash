...
| [Workspace navigator V2](ui-mockups/codex/review-v2/02-workspace-navigator.html) | Automatically detected repository/folder groups; a first-class Remote SSH connection; Idle and Unknown retained; plus the production header, compact vendor/machine badges, per-card cost, a shared usage summary, and card-level resume readiness. |
...
- [Workspace Navigator V2](ui-mockups/codex/review-v2/02-workspace-navigator.html) now uses the production coSlash header vocabulary: logo and tagline, per-machine health, Diagnostics, and Settings. The redundant page title is removed, while Remote SSH becomes a first-class sidebar connection.
- The oversized board rollup is replaced by one compact result line shared by List and Board. It reports session count, time scope, vendor split, tokens, estimated cost, and unavailable pricing in the same place.
- Board cards follow the production compact-card hierarchy: vendor and machine badges, title, summary, cost, branch, and updated time. Resume readiness remains a quiet footer with the decisive context/cache signal.
...
**September 17 — production-alignment board pass:** verified Workspace Navigator at 1265 × 863 and 390 × 844. The shared result line reconciled 21 sessions, 36.5M sample tokens, ≈$191.66 plus unavailable pricing, and a six/fifteen Claude Code/Codex split in both Board and List. All 21 sessions retained visible per-card costs in Board, including the unavailable case; Unknown activity remained separate. Remote SSH narrowed the view to five sessions, 15.2M tokens, and ≈$34.45. The production-style Settings and Diagnostics controls opened, the redundant page heading remained absent, and both document widths matched their viewports. No browser console warnings or errors were captured. These figures and recommendations are curated prototype data.
