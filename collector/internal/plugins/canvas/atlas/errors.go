...
// ErrAtlas marks every failure this package produces so callers can classify a
// wrapped error without depending on the concrete type.
var ErrAtlas = errors.New("atlas: operation failed")
...
// contents and private absolute paths never reach Message.
type Error struct {
	Code    string
	Message string
	// Field is the dotted path of the offending value, for example
	// "components[1].seat.model". Empty when the failure is not field-scoped.
	Field string
...
func (e *Error) Is(target error) bool { return target == ErrAtlas }
