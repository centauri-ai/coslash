...
Raw rollouts are streamed to a private spool while their hashes and lengths
are computed. After the live source passes its post-copy fingerprint check,
the parser runs again against those frozen rollout files; processed records
and exact changes therefore describe the same bytes that the bundle retains.
The spool survives collector restarts and remains available through `Open` and
bounded `Read` calls until `Discard` is explicit. The spool root and bundle
directories are mode `0700`; artifact files and manifests are mode `0600`.
...
machine-wide configuration. Capture errors use stable blocker codes and must
not include source content, paths, or identities in logs or diagnostics.
