...
func TestMarshalAppliesAggregateReductionStages(t *testing.T) {
	...
	t.Run("commits", func(t *testing.T) {
		...
		for i := range local.Commits {
			local.Commits[i] = strings.Repeat("c", maxCommitTextBytes)
		}
		decoded := marshalAggregate(t, local)
		if len(decoded.Session.Commits) >= len(local.Commits) {
			t.Fatal("commits were not reduced")
		}
	...
	t.Run("file edits", func(t *testing.T) {
		...
		decoded := marshalAggregate(t, local)
		if len(decoded.Session.FileEdits) >= len(local.FileEdits) || decoded.Session.FileEdits[0].Path != local.FileEdits[0].Path {
			t.Fatal("file edits were not reduced from the tail")
		}
