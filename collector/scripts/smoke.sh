...
if [[ "$mode" == "embedded" ]]; then
  ...
  settings='{"$schema":"https://raw.githubusercontent.com/centauri-ai/coslash/main/settings.schema.json","version":1,"synthesis":{"enabled":false,"backend":"claude-cli","model":"claude-haiku-4-5"},"appearance":{"theme":"light"},"launch":{"terminal":"terminal"},"remote":{"id":"r_0123456789abcdef","sshAlias":"coslash-smoke-invalid","enabled":false}}'
  actual=$(
    printf 'header = "X-Coslash-Token: %s"\n' "$token" |
